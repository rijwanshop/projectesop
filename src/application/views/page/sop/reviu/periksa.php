<?php if($this->uri->segment(3) != ''){
	...
	foreach($sop->result_array() as $row){
		...
		$userpengesah = $row['sop_pengesah_user'];
		...
		$alias = $row['sop_alias'];
		$link = $row['sop_update_file'];
		
		$Arr['konektor'][$t] = $row['sop_konektor'];
	...
	// gambar ttd
	$g = $this->db->query("select ttd_pengesah_gambar from ttd_pengesah where user_id='".$userpengesah."'")->row_array();;
	$gambar = $g['ttd_pengesah_gambar'];
	...
	// reviu
	$reviu = $this->Sop_m->reviu_detail($this->uri->segment(4),$this->uri->segment(6));
	if($reviu->num_rows() > 0){
		foreach($reviu->result_array() as $row){
			$msg = ($row['reviu_status'] == 'ditolak' ? 'danger' : 'success');
			$revstatus = $row['reviu_status'];
			$revcatatan = $row['reviu_catatan'];
			$revtanggal = $row['reviu_tanggal_tindakan'];
		}
	}
			...
			<div class="col-lg-12">
			  ...
			  <div class="panel">
				<div class="panel-body">
					<?php if($action['notif_action'] == 'belum'){ ?>
							
							<div class="text-left col-md-12">
							 <?=($link == '' ? '' : '<a href="'.base_url().'sop/pdf_sop/'.$row['sop_alias'].'" class="btn btn-info btn-xs" target="_blank">Pdf Manual</a>')?>
							</div>
							<div class="text-right col-md-12 Button">
							  <button type="submit" class="btn btn-danger" id="Salah">
									<i class="icon wb-arrow-left white icon-circle" aria-hidden="true"></i> Masih Salah
							  </button>
							  <button type="submit" class="btn btn-success" onclick="Benar(<?=$alias?>,<?=$this->uri->segment(5)?>,<?=$this->uri->segment(6)?>)">
									<i class="icon wb-check white icon-circle" aria-hidden="true"></i> Sudah Benar
							  </button>
							</div>
							<form class="form-horizontal" id="FrmAjax" style="display:none">
								<input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>" />
								<input type="hidden" name="id" value="<?=$this->uri->segment(5)?>"/>
								<input type="hidden" name="reviuid" value="<?=$this->uri->segment(6)?>"/>
								<input type="hidden" name="alias" value="<?=$alias?>"/>
									<div class="Errors"></div>
									<div class="form-group row">
									  <label class="col-md-3 form-control-label">Catatan</label>
									  <div class="col-md-6">
							...
							function Benar(alias,id,reviu){
								   if (confirm('Yakin sudah benar?')) {
										$.ajax({
											url: '<?=base_url()?>notification/benar',
											type: "POST",
											data: {i: id, a: alias, r: reviu, '<?php echo $this->security->get_csrf_token_name(); ?>' : '<?php echo $this->security->get_csrf_hash(); ?>'},
											success: function () {
											  alert('Data Berhasil Disimpan');
											  location.href="<?=base_url()?>notification/semua"
											}
										});
									}}
							</script>	
					<?php }else{
						echo'<div class="alert alert-'.$msg.' alert-dismissible" role="alert">
						  Tanggal: '.tgl_indo2($revtanggal).'<br>
						  Status: '.$revstatus.'<br>
						  Catatan: '.$revcatatan.'<br>
						</div>';
					} ?>
				</div>
				<div class="panel-body">
									...
									<td width="50%" align="center">
										...
										<div style="border-bottom:1px solid #000; text-align:left; margin-left:-2px; margin-right:-2px; padding:2px 0px;">
											...
											<div style="float:left; width:450px"><?=$disahkan_jabatan?> <br><br>
												<?=($gambar != '' ? '<img src="'.site_url().'master/img_ttd/'.$gambar.'" height="25">' : '<br>')?><br><br>
												<?=$disahkan_nama?> <br><?=$disahkan_nip?>
											</div>
											<div style="clear:both"></div>
										</div>
						...
						<table cellpadding="2" cellspacing="0"  border="1" align="center" class="TableKeg" style="margin-top:20px; width:32cm; font-size:9px;; font-family:arial; color:#000">
								<tr bgcolor="#ddd" align="center">
									...
									<tr>
											...
											<td align="center">
												<?php
													if(substr($Arr['pelaksana'.$q.''][$i], -1) == 'Y'){
														...
														if($Arr['konektor'][$no] != ''){
															$query = $this->Sop_m->img_simbol('k-atas-'.$Arr['pelaksana_perbaris'][$i].'-'.$Arr['pelaksana_perbaris'][$no].'');
															echo '<img src="'.base_url().'assets/media/simbol/'; if($query){echo $query[0]['simbol_img'];} echo '" style="position:absolute; '; if($query){echo $query[0]['simbol_margin'];} echo '">'; 
															
														}

														if($Arr['konektor'][$i] != ''){
															echo '<img src="'.base_url().'assets/media/simbol/penghubung-bawah.png" style="position:absolute; margin-top:-35px; margin-left:-26px">'; 
														}else{
															if($i > 0){
																if(substr($Arr['decision'][$i],-1) == 'Y'){
										...
										<?php } ?>	
										<td><div style="height:135px; overflow:hidden;"><div style="height:135px; word-wrap: break-word; display: table-cell; vertical-align: middle"><?=nl2br($Arr['kelengkapan'][$i])?></div></div></td>
										<td><div style="height:135px; overflow:hidden;"><div style="height:135px; word-wrap: break-word; display: table-cell; vertical-align: middle"><?=nl2br($Arr['waktu'][$i])?></div></div></td>
										<td><div style="height:135px; overflow:hidden;"><div style="height:135px; word-wrap: break-word; display: table-cell; vertical-align: middle"><?=nl2br($Arr['hasil'][$i])?></div></div></td>
										<td><div style="height:135px; overflow:hidden;"><div style="height:135px; word-wrap: break-word; display: table-cell; vertical-align: middle"><?=nl2br($Arr['keterangan'][$i])?></div></div></td>
									</tr>
									<?php } ?>	
						</table>
...
<script>
			...
			$("#FrmAjax").on('submit',(function(e) {
				e.preventDefault();
				$.ajax({
				url: "<?=base_url()?>notification/salah", 
