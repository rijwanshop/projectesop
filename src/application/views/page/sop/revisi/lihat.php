<?php if($this->uri->segment(3) != ''){
	...
	// catatan revisi
	$revisi = $this->Sop_m->revisi_detail($this->uri->segment(4),$this->uri->segment(5));
	if($revisi->num_rows() > 0){
		foreach($revisi->result_array() as $row){
			$msg = ($row['revisi_status'] == 'ditolak' ? 'danger' : 'success');
			$revisitanggal = ($row['revisi_tanggal'] != '0000-00-00' ? tgl_indo2($row['revisi_tanggal']) : '');
			$revisistatus = $row['revisi_status'];
			$revisicatatan = $row['revisi_catatan'];
			$revisicatatanadmin = ($row['revisi_status'] == 'pending' ? 'Belum dilakukan tindakan oleh admin' : $row['revisi_catatan_admin']);
			$revisialias = $row['sop_revisi_alias'];
		}
	}
	...
	$Arr = array();
	$t = 0;
	$sop = $this->Sop_m->sop_revisi_detail($revisialias);
	if($sop->num_rows() > 0){
					...
					<div class="panel-body">
						<div class="row">
							...
							<?php if($revisi->num_rows() > 0){ ?>
								<div class="errors alert alert-<?=$msg?> alert-dismissible">
									Tanggal Revisi : <?=$revisitanggal?><br>
...
<script>
			...
			$("#FrmAjax").on('submit',(function(e) {
				e.preventDefault();
				$.ajax({
				url: "<?=base_url()?>act_sop/add_revisi", 
				...
				success: function(data)   // A function to be called if request succeeds
				{
					if(data == '1'){
						...
						location.href="<?=base_url()?>sop/revisi_sop"
					}else{
