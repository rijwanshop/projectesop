<?php if($this->uri->segment(3) != ''){
	...
	// gambar ttd
	$g = $this->db->query("select * from ttd_pengesah where user_id='".$userid."'")->row_array();;
	$ttdnama = $g['ttd_pengesah_nama'];
	$ttdnip = $g['ttd_pengesah_nip'];
	$ttdgambar = $g['ttd_pengesah_gambar'];
	$ttdjabatan = $g['ttd_pengesah_jabatan'];
			...
			<div class="col-lg-12">
			  ...
			  <div class="panel">
				...
				<div class="text-left col-md-12">
				 <?=($link == '' ? '' : '<a href="'.base_url().'sop/pdf_sop/'.$row['sop_alias'].'" class="btn btn-info btn-xs" target="_blank">Pdf Manual</a><br><br>')?>
				</div>
				...
				<input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>" />
								...
								<tbody><tr valign="top">
									...
									<td width="50%" align="center">
										...
										<div style="border-bottom:1px solid #000; text-align:left; margin-left:-2px; margin-right:-2px; padding:2px 0px;">
											...
											<div style="float:left; width:10px;">:</div>
											<div style="float:left; width:450px"><span id="divJabatan"><?=$disahkan_jabatan?></span> <br><br>
												<div id="bg_ttd" style="background:#eee; width:170px; height:70px; padding:2px; text-align:center; line-height:70px">
													<input id="disahkan" name="disahkan" type="checkbox" value="">
													<input name="nama" id="nama" type="hidden" value="<?=$nama?>">
													<input name="jabatan" id="jabatan" type="hidden" value="<?=$disahkan_jabatan?>">
													<input name="nip" id="nip" type="hidden" value="<?=$disahkan_nip?>">
													<input name="no_alias" type="hidden" value="<?=$alias?>">
													<input name="gambar" type="hidden" value="<?=$ttd?>">
												</div>
												<span id="divNama"><?=$disahkan_nama?></span> <br><span id="divNip"><?=$disahkan_nip?></span>
											</div>
											<div style="clear:both"></div>
										</div>
						...
						<table cellpadding="2" cellspacing="0"  border="1" align="center" class="TableKeg" style="margin-top:20px; width:32cm; font-size:9px;; font-family:arial; color:#000">
								<tr bgcolor="#ddd" align="center">
									...
									<tr>
										...
										<?php } ?>	
										<td><div style="height:135px; overflow:hidden;"><div style="height:135px; word-wrap: break-word; display: table-cell; vertical-align: middle"><?=$Arr['kelengkapan'][$i]?></div></div></td>
										<td><div style="height:135px; overflow:hidden;"><div style="height:135px; word-wrap: break-word; display: table-cell; vertical-align: middle"><?=$Arr['waktu'][$i]?></div></div></td>
										<td><div style="height:135px; overflow:hidden;"><div style="height:135px; word-wrap: break-word; display: table-cell; vertical-align: middle"><?=$Arr['hasil'][$i]?></div></div></td>
										<td><div style="height:135px; overflow:hidden;"><div style="height:135px; word-wrap: break-word; display: table-cell; vertical-align: middle"><?=$Arr['keterangan'][$i]?></div></div></td>
									</tr>
									<?php } ?>	
						</table>
						...
						<?php } ?>
							<button type="submit" class="btn btn-success" style="float:right; margin:5px 0 20px 0; width:120px; height:40px">Simpan</button>
				</form>	  
				</div>
			  </div>
...
<script>
			...
			$("#FrmAjax").on('submit',(function(e) {
				e.preventDefault();
				$.ajax({
				url: "<?=base_url()?>act_sop/add_disahkan", 
				...
				success: function(data)   // A function to be called if request succeeds
				{
					if(data == '1'){
						...
						location.href="<?=base_url()?>sop/pengesahan_sop"
					}else{
						  $('.alert').show();
			...
			$("#disahkan").on('click',function(){
							
				if($('#disahkan:checkbox:checked').length == 0){ 
					$('#bg_ttd').css({
						'background': '',
					...
					$('#bg_ttd').css({'background': 'url(<?=site_url()?>master/img_ttd/<?=$ttdgambar?>) no-repeat center'});
					$('#disahkan').val('DISAHKAN');
					$('#divNama').html('<?=$ttdnama?>');
					$('#divNip').html('<?=$ttdnip?>');
					$('#divJabatan').html('<?=$ttdjabatan?>');
					$('#nama').val('<?=$ttdnama?>');
					$('#nip').val('<?=$ttdnip?>');
					$('#jabatan').val('<?=$ttdjabatan?>');
				}
			});
</script>
