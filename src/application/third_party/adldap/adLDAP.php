...
class adLDAP {
     ...
     * @var string
     */
    protected $accountSuffix = "OU=Aplikasi,OU=KEMSETNEG,DC=SETNEG,DC=GO,DC=ID";
     ...
     * @var string
     */
    protected $baseDn = "DC=SETNEG,DC=GO,DC=ID";
     ...
     * @var array
     */
    protected $domainControllers = array("192.168.1.77");
     ...
     * @var string
     */
    protected $adminUsername = "thereal.sambel";
    protected $adminPassword = "janjiJiwa#TeruntukTem4nS3j!w4";
    ...
    public function connect() {
        ...
        //$ldaprdn  = 'cn=180005392,ou=ddki,ou=kemsetneg,dc=setneg,dc=go,dc=id'; // ldap rdn or dn
        ...
        if ($this->adminUsername !== NULL && $this->adminPassword !== NULL) {
            ...
            $this->ldapBind = @ldap_bind($this->ldapConnection, 'cn=thereal.sambel,ou=aplikasi,ou=kemsetneg,dc=setneg,dc=go,dc=id', 'janjiJiwa#TeruntukTem4nS3j!w4');
            if (!$this->ldapBind) {
    ...
    public function authenticate($username, $password, $preventRebind = false) {
		...
		//$this->ldapBind = @ldap_bind($this->ldapConnection, 'cn='.$username.',ou=kemsetneg,dc=setneg,dc=go,dc=id', 'yusri@123');
        if (!$this->ldapBind) {
			$ret = false;
        }
